...
- (void)tappedInToolbar:(ThumbsMainToolbar *)toolbar showControl:(NSInteger)control
{
    ...
    //북마크 선택시..
   // NSLog(@"bookCnt  :%d",[bookmarked count]);
...
/*              
        case 1:
            NSLog(@"Outline list Call~~~");
          
            outlineViewController = [[OutlineViewController alloc] init];//initWithReaderDocument:document];            
            outlineViewController.title = self.title;
            
            outlineViewController.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
            outlineViewController.modalPresentationStyle = UIModalPresentationFullScreen;
           
            NSArray *pdfs = [[NSBundle mainBundle] pathsForResourcesOfType:@"pdf" inDirectory:nil];
            NSString *documentName = [[pdfs lastObject] lastPathComponent]; assert(documentName != nil);
            //            [outlineViewController loadFromFileName:documentName];
            kNewsletterAppDelegate * appDelegate = (kNewsletterAppDelegate*)[[UIApplication sharedApplication] delegate];
            [outlineViewController loadFromPDFDocument:[appDelegate getDocument]];
            
            [self presentModalViewController:outlineViewController animated:NO];
            
            [outlineViewController release]; // Release OutlineViewController   
            
            
//            outlineViewController = [[OutlineViewController alloc] init];
//            NSArray *pdfs = [[NSBundle mainBundle] pathsForResourcesOfType:@"pdf" inDirectory:nil];
//            NSString *documentName = [[pdfs lastObject] lastPathComponent]; assert(documentName != nil);
////            [outlineViewController loadFromFileName:documentName];
//            id appDelegate = [[UIApplication sharedApplication] delegate];
//            [outlineViewController loadFromPDFDocument:[appDelegate getDocument]];
// //           [outlineViewController loadFromPDFDocument:(CGPDFDocumentRef) PDFDocument];
//            [appDelegate pushViewController:outlineViewController];
...
//버튼 숨기기.
- (void)setHiddenToolbarBtn {
    
    mainToolbar._rightBtn.hidden = YES;
